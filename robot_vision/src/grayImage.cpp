...
private:
    ...
    std::string sub_image_path; //訂閱圖像的路徑


public:
    RGB_GRAY():it_(nh_), nh_("~")//构造函数
    {
        nh_.getParam("sub_image_path",sub_image_path);
        ROS_INFO_STREAM(sub_image_path);

        image_sub_ = it_.subscribe(sub_image_path, 1, &RGB_GRAY::convert_callback, this); //定义图象接受器，订阅话题是“camera/rgb/image_raw”
        image_pub_ = it_.advertise("image_gray", 1); //定义图象发布器
        //初始化输入输出窗口
    ...
    */
    ...
    {
       cv::Mat img_out, img_hsv, img_gray;
       cv::cvtColor(img, img_hsv, CV_BGR2HSV); //GRB 轉 HSV
       cv::Mat hsv_channels[3];
       cv::split(img_hsv, hsv_channels);
       // threshold(img_gray, img_out, 70, 255.0, cv::THRESH_BINARY_INV); //二值化區分
       threshold(hsv_channels[2], img_out, 200, 255.0, cv::THRESH_BINARY);
       // cv::imshow(INPUT, img);
